...
protected:
	...
	struc_base<T>& operator_diy(const struc_base<T>& m, std::function<void(T&, const T&)>f);
public:
	adjacent_list_matrix(int row, int col, const T& default_value = 0) :
		pmtx(
			std::make_shared<matrix_struc_type>(row, row_type())
		), col_size(col),
		default_value(default_value) {}

	adjacent_list_matrix(const adjacent_list_matrix& a) :
		pmtx(std::make_shared<matrix_struc_type>(*a.pmtx)),
		col_size(a.col()), default_value(a.default_value) {}
	...
	virtual const T& get(int r, int c)const override	{
		return cget(r, c);
	}
...
template<typename T>
struc_base<T>& adjacent_list_matrix<T>::operator_diy(const struc_base<T>& m, std::function<void(T&, const T&)>f) {
...
auto adjacent_list_matrix<T>::_get(int row, int col)const {
	...
	auto iter = std::find_if((*pmtx)[row].begin(), (*pmtx)[row].end(),
